...
void  show_hz(int x, int y, unsigned char *S)
 { int offset, color = getcolor();
   setcolor(gethz_color());
   offset =  ((int)S[0]-161)*94 + (S[1]-161) ;
   xshow_hz(x, y, offset);
   setcolor(color);
 }

void  show_hzs(int x, int y, char *S)
 { int K=0;
   struct linesettingstype  stype;
   getlinesettings(&stype);
   sethz_file(hzk16fp);  sethz_fx(X_ROUND);  sethz_dot(16);
   sethz_zoomxy(1, 1);   sethz_speed(0);
   while(S[K])
     { if(S[K]>0)  { x+=8; K++; continue; }
       show_hz(x+K*8, y-16, S+K);  K+=2;
     }
   setlinestyle(stype.linestyle, stype.upattern, stype.thickness);
 }

void  xshow_hz(int x, int y, int begin)
 { int j , K=0 ;
   unsigned char buffer[32];
   fseek(hz_fp, (long)begin*HZ.text_dot/8*HZ.text_dot-ftell(hz_fp), SEEK_CUR);
   fread(buffer, 32, 1, hz_fp);
   for(j=0; j<16; j++)
    { setlinestyle(4, ((unsigned)buffer[K++]<<8)|buffer[K++], 1);
      line(x+15, y+j, x, y+j);
    }
   setlinestyle(0, 0, 1);
 }
